...
for h in f2:
	...
	md_tmp = re.findall("MD:Z:\w+", " ".join(ele[11:])) 
	md = md_tmp[0].split(":")[2]
	if (read_name in list(mydic.values())[0] or read_name in list(mydic.values())[1] ) and  ('D' not in cigar and 'I' not in cigar and 'P' not in cigar and 'X' not in cigar and md.isdigit( ) ):
	#if (read_name in list(mydic.values())[0] ) and  ('D' not in cigar and 'I' not in cigar and 'P' not in cigar and 'X' not in cigar and md.isdigit( ) ):
		test.append(read_name)
#print(len(test))
#print(len(set(test)))
#print(set(test)-list(mydic.values())[0] )
